...
| Pruebas de dominio | `npx vitest run` *(Vitest aún no está instalado; se agrega con la primera feature de dominio)* |
...
- No hay autenticación real: un **selector en la UI** elige usuario mock, rol y empresa (tenant).
- Roles (SDD §14): **Maker**, **Checker**, **Admin de Plantillas**, **Auditor** (solo lectura),
  **Operador de Soporte** (solo DLQ).
...
- **Vitest, solo para `src/domain/`.** Cobertura mínima: balance (RD-03), hash de idempotencia
